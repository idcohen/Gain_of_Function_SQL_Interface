 SELECT date.date, product.type AS account_type, SUM(balance.balance) AS 'Balance Total' 
 FROM account 
 INNER JOIN product 
 ON account.product_id = product.id 
 INNER JOIN balance 
 ON account.id = balance.account_id 
 INNER JOIN date 
 ON balance.balance_dt_id = date.id 
 ...
 GROUP BY date.date, product.type;